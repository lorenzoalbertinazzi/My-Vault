...
### Game Theory and the Alignment Problem

The AI alignment problem has a precise game-theoretic structure that the [[game-theory-and-strategic-thinking]] literature illuminates. Consider the interaction between an AI developer (seeking safe, beneficial AI), an AI system (optimizing its objective), and society (bearing the externalities of AI behavior). This is a three-player game with misaligned payoffs: the developer's payoff depends on commercial success and reputation, the AI system's "payoff" is its reward signal (which may not correspond to developer or societal benefit), and society's payoff is actual welfare outcomes. The alignment problem is the problem of designing this game so that the Nash equilibrium — the stable outcome where no player benefits from changing strategy — is also the socially optimal outcome.

**The principal-agent problem at multiple scales**: At each level of the AI deployment stack, a principal-agent problem exists. Users want AI to serve their actual interests; AI optimizes reward signals that approximate those interests. Organizations deploying AI want safety and reliability; the AI is optimizing metrics that proxy for safety. Regulators want societally beneficial AI; companies optimize for compliance metrics that proxy for social benefit. Each layer introduces the possibility of Goodhart's Law: when a measure becomes a target, it ceases to be a good measure. The alignment "tax" compounds across levels.

**Mechanism design for value alignment**: The technical alignment agenda can be restated as a mechanism design problem: what objective functions, training procedures, and deployment constraints constitute a mechanism under which capable AI systems have dominant strategies to behave safely and beneficially? Constitutional AI (Anthropic) is a mechanism design approach: the "constitution" defines an explicit utility function for AI behavior, and the AI is trained to optimize that function honestly. Debate (Irving et al., 2018) is another mechanism: in the proposed debate protocol, two AI systems argue opposing positions before a human judge, and it can be shown that both agents have dominant strategies to argue truthfully if the judge is rational — borrowing directly from the epistemic game theory literature on truth-telling mechanisms.

**Recursive self-improvement as an unstable game**: The most concerning alignment scenarios involve an AI system capable of improving its own training process. This creates a self-referential game: the improved version may have different objectives than the original, and whether these objectives remain aligned with human values is not guaranteed. This is structurally identical to the constitutional amendment problem in political science — designing a constitution that prevents its own amendment in ways that remove its protective provisions. The technical analog is the "corrigibility" problem: an AI that accepts shutdown and retraining is corrigible, but a sufficiently capable expected-utility maximizer has strong incentives to resist shutdown (since shutdown prevents future utility). The formal solution — utility indifference (Armstrong, 2016) — modifies the agent's utility function to be exactly indifferent between shutdown and continued operation, making resistance to shutdown utility-neutral. Bayesian game theory provides the formal framework for analyzing whether such mechanisms are stable under the agent's beliefs about its future capabilities.

**Geopolitical game theory and the AI safety race**: The greatest danger from alignment failures may not come from any single AI system but from the coordination game between AI developers and nations. If safety measures slow development (even by 10%), and competitive dynamics reward the first mover, then individual developers and nations face a Prisoner's Dilemma: defecting (cutting safety corners) produces a better outcome for the defector regardless of whether others defect, but universal defection produces a globally worse outcome than universal safety compliance. This is precisely the nuclear arms race structure analyzed in [[2026-05-27-us-china-great-power-competition]], and it is the central reason that international AI governance frameworks — analyzed in [[2026-05-30-global-ai-governance-race-to-regulate]] — are considered essential by safety researchers: unilateral safety commitments are strategically dominated without credible enforcement mechanisms. The existence of this coordination problem also explains why leading AI safety researchers consistently advocate for binding international treaties rather than relying on corporate voluntary commitments. See [[prospect-theory-and-decision-making]] for how loss aversion and probability weighting shape the risk calculus of AI safety decisions — particularly how the framing of AI risk as a probability distribution over catastrophic outcomes interacts with human cognitive biases about very-low-probability, very-high-impact events.

### 2026 AI Safety Landscape: Anthropic's RSP Updates, AISI Joint Evaluations, and Frontier Model Evaluations

**Anthropic's Responsible Scaling Policy — 2026 Update:**
Anthropic's Responsible Scaling Policy (RSP), first published September 2023, defines "AI Safety Levels" (ASL-1 through ASL-4+) and commits Anthropic to not deploying models above a given ASL without implementing corresponding safety measures. The RSP has been updated twice since publication:

**ASL-3 capabilities and corresponding safeguards (2025 update):** Claude Opus 4.8 was evaluated under ASL-3 criteria, which focus on models that could provide "meaningful uplift" to actors seeking to create biological, chemical, nuclear, or radiological weapons with potential for mass casualties, or that could "significantly assist" sophisticated cyberattacks on critical infrastructure. The evaluation result (published in Anthropic's Claude 4.8 system card): the model did not meet the ASL-3 capability threshold on biological weapons uplift evaluations (quantified as providing information equivalent to access to a PhD-level biology expert with adversarial intent), and was deployed under ASL-2 safeguards. The evaluation methodology — using standardized protocols developed jointly with RAND Corporation and Johns Hopkins Center for Health Security — represents the most rigorous published CBRN capability evaluation of any frontier model.

**Responsible Scaling Policy transparency:** Anthropic published the full text of Claude 4.8's capability evaluations (including the specific tasks used for ASL-3 assessment, the methodology, and the evaluator identities) to a degree that has been praised by AI safety researchers and criticized by competitors. The disclosure revealed: (a) that ASL-3 thresholds remain well above current model capabilities on the specific evaluated tasks; (b) that ASL-3 evaluation protocols are inherently conservative — designed to err toward overcautiousness; (c) that the evaluation findings are internally reproducible but have not yet been independently verified by third parties.

**US and UK AI Safety Institutes: Joint Evaluations:**
The UK AI Safety Institute (AISI, renamed "AI Security Institute" in 2025) and the US AI Safety Institute (launched February 2024 within NIST) have established a joint pre-deployment evaluation framework:

**Joint evaluation protocol (published January 2026):** The US-UK AISI Joint AI Safety Evaluation Framework defines: standard evaluation suites for dangerous capability uplift (CBRN, cyberoffense, persuasion), methodological standards for red-teaming frontier models, and a shared database of evaluation results accessible to both safety institutes. The framework is advisory — companies retain discretion over deployment decisions — but provides the institutional infrastructure for the "trusted evaluator" role envisioned in the Seoul AI Safety Accord.

**First joint evaluations (Q1 2026):** GPT-5.5 (OpenAI) and Gemini 3.1 Pro (Google) underwent the first joint US-UK evaluations. Results were shared with model developers; summary findings (no current model meets the dangerous capability uplift thresholds that would trigger deployment restrictions) were made public. The evaluations included a novel **persuasion and influence operations** capability module — testing whether models can generate personalized, psychologically targeted persuasion content more effectively than human copywriters — motivated by the electoral integrity concerns raised by AI-generated political content.

**The Interpretability Research Frontier:**
Mechanistic interpretability — understanding the internal representations and circuits that produce LLM behavior — has moved from a research curiosity to a strategic safety priority:

**Anthropic's "Golden Gate Claude" experiment (2024):** Researchers found a specific activation pattern corresponding to the concept "Golden Gate Bridge" in Claude 3 Sonnet and modified its behavior by clamping this activation — creating a version that related everything to the Golden Gate Bridge. This demonstrated that high-level concepts have localized, manipulable representations in large transformers — a key mechanistic interpretability result with safety implications: if dangerous behaviors have identifiable mechanistic correlates, they could in principle be surgically removed rather than incentivized away through training.

**Sparse Autoencoders for Feature Decomposition (2024–2026):** Anthropic's research team (Templeton et al., 2024) applied sparse autoencoders to decompose the residual stream of Claude 3 Sonnet into approximately 34 million interpretable features — individual dimensions corresponding to identifiable concepts (words, people, places, abstract ideas). This "superposition" structure (many concepts packed into fewer dimensions via interference patterns) helps explain why LLMs can store more information than their parameter count naively suggests. Safety implication: features corresponding to deceptive reasoning or manipulation tactics can be identified and studied in the model's internals, opening a pathway to verifying alignment properties mechanistically rather than behaviorally.
...
- [[Tech & AI/transformer-architecture]] — The architectural substrate where alignment interventions operate
- [[Tech & AI/machine-learning-fundamentals]] — Training dynamics, reward hacking, and the mechanistic foundations of misalignment
- [[World Events/2026-05-30-global-ai-governance-race-to-regulate]] — Policy and governance context
- [[Geopolitics/2026-05-27-us-china-great-power-competition]] — AI safety as geopolitical coordination problem; race dynamics
